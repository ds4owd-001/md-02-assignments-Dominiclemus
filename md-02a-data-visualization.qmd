...
gapminder__summary_2007 <- gapminder_2007 |> 
  group_by(continent) |> 
  summarise(
    count = n(),
...
**"geom_bar()" function** makes the height of the bars proportional to the number of observations of the variable on the x-axis in the gap_minder object.

The "geom_col() function however makes the height of the bars proportional to the data values in the gap_minder_2007 object(Thus for the"geom_col()" function, the y-variable values must be specified in the data objest ). These two sets of observations and datad values being the same, the two plots were the same.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    geom_histogram() function just like the geom_bar() function does not require specifying the dependent(y-) variable as it plots the number of observations of the x-variable in the object
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    The "breaks" argument helps to create a regularly spaced sequence between numbers for example Breaks = seq(40,85,2.5) means that start the values from 40, end at 85 with an increment(interval) of 2.5 each time ( 40, 42.5, 45, ........, 85)
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    The "country_colors" shows hexadecimal color codes/schemes for countries and continents which describe the different amounts of red, green and blue in each color
